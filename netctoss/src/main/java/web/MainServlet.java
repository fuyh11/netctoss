...
public class MainServlet extends HttpServlet {
    ...
    protected void login(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        //校验验证码
        HttpSession sn = req.getSession();
        String imgCode = (String) sn.getAttribute("imgCode");
        ...
        if (a == null) {
            ...
            //将账号记录到Cookie中，
            //便于其他功能页面上显示账号。
            Cookie ck = new Cookie("adminCode", adminCode);
            res.addCookie(ck);
            //将账号存入session，便于
            //后续做登录检查的业务。
            sn.setAttribute("adminCode", adminCode);
            //成功，重定向到主页
    ...
    protected void addCost(HttpServletRequest req, HttpServletResponse res) throws IOException {
        req.setCharacterEncoding("utf-8");
        //接收请求数据
        String name = req.getParameter("name");
        String baseDuration = req.getParameter("baseDuration");
