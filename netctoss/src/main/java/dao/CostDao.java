...
public class CostDao {
    ...
    public void save(Cost cost) {
        ...
        try {
            con = DBUtil.getConnection();
            String sql = "insert into cost " + "values(null,?,?,?,?,'0',?,sysdate,null,?)";
            PreparedStatement ps = con.prepareStatement(sql);
            ...
            e.printStackTrace();
            throw new RuntimeException("新增资费失败");
        } finally {
            DBUtil.close(con);
        }
    }
    ...
    public void update(Cost cost) {
        ...
        try {
            con = DBUtil.getConnection();
            String sql =
                "update cost set " + "	name=?," + "	base_duration=?," + "	base_cost=?," + "	unit_cost=?,"
    ...
    public List<Cost> findByPage(int page, int size) {
        ...
        try {
            con = DBUtil.getConnection();
            String sql = "select * from (" + "	select c.*,@rownum:=@rownum+1 AS r  from ("
                + "	  select * from cost,(SELECT @rownum:=0) r " + "	  order by cost_id" + " ) c"
